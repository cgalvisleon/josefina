...
* @return error
**/
func (s *Node) setRecord(schema, model, key string) error {
	if !s.started {
		return fmt.Errorf(msg.MSG_NODE_NOT_STARTED)
	}
	...
	if leader != s.host {
		err := methods.setRecord(leader, schema, model, key)
		if err != nil {
			return err
		}
...
* @return error
**/
func (s *Node) deleteRecord(schema, model, key string) error {
	if !s.started {
		return fmt.Errorf(msg.MSG_NODE_NOT_STARTED)
	}
	...
	if leader != s.host {
		err := methods.deleteRecord(leader, schema, model, key)
		if err != nil {
			return err
		}
